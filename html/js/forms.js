...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //console.log("Stored cookie: " + storedCookie);
    ...
    if (socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify({type: 'login', data: {cookie: storedCookie}}));
  ...
  document.getElementById('login-form').addEventListener('submit', function(e) {
    ...
    socket.send(JSON.stringify({type: 'login', data: data}));
  ...
  document.querySelector('.register-form').addEventListener('submit', function(e) {
    ...
    socket.send(JSON.stringify({type: 'register', data: data}));
  });
  ...
  document.getElementById('logout-form').addEventListener('submit', function(e) {
    ...
    socket.send(JSON.stringify({type: 'logout', data: data}));
  ...
  socket.onmessage = function(event) {
    ...
    if (message.type === 'loginResponse' && message.data.login === "true") {
      ...
      //delete CurrentUser from session storage
      ...
      // delete the cookie from session storage
      sessionStorage.removeItem('cookie');
      ...
      socket.send(JSON.stringify({type: 'loggedInRequest'}));
      }
